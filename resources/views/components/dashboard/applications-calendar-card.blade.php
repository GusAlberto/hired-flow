...
<section x-data="window.applicationsCalendarCard ? window.applicationsCalendarCard() : {}" class="mb-8">
    ...
    <button type="button" @click="open = !open"
        ...
        <div class="relative flex flex-wrap items-start justify-between gap-6">
            <div>
                ...
                <p class="mt-1 text-sm text-slate-600">Click this card to expand a full monthly view and see jobs
                    applied on each date.</p>
            </div>
            ...
            <div class="relative flex items-start gap-3">
                ...
                <div class="rounded-2xl border border-cyan-200 bg-white/90 px-4 py-3 shadow-sm">
                    ...
                    <div class="mt-2 grid grid-cols-7 gap-1">
                        ...
                        <div class="h-2.5 w-2.5 rounded-sm bg-slate-300"></div>
                        <div class="h-2.5 w-2.5 rounded-sm bg-slate-300"></div>
                        <div class="h-2.5 w-2.5 rounded-sm bg-cyan-500"></div>
                        <div class="h-2.5 w-2.5 rounded-sm bg-slate-300"></div>
                        <div class="h-2.5 w-2.5 rounded-sm bg-slate-300"></div>
                        <div class="h-2.5 w-2.5 rounded-sm bg-slate-300"></div>
                        <div class="h-2.5 w-2.5 rounded-sm bg-slate-300"></div>
        ...
        <div class="relative mt-4 flex items-center gap-2 text-sm font-semibold text-cyan-700">
            ...
            <svg class="h-4 w-4 transition group-hover:translate-x-1" viewBox="0 0 24 24" fill="none"
                aria-hidden="true">
    ...
    <div x-show="open" x-cloak x-transition.opacity.duration.180ms @keydown.escape.window="open = false"
        ...
        <div class="w-full max-w-xl rounded-3xl border border-cyan-200 bg-white p-4 shadow-xl">
            ...
            <div
                class="mb-2 grid grid-cols-7 gap-2 text-center text-[11px] font-bold uppercase tracking-wide text-slate-500">
            ...
            <div class="grid grid-cols-7 gap-2">
                <template x-for="cell in monthCells" :key="cell.key">
                    <button type="button" @click="cell.iso && selectDate(cell.iso)" :disabled="!cell.iso"
